...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        myScrollView = [[UIScrollView alloc]initWithFrame:CGRectMake(UUYLabelwidth, UULabelHeight, frame.size.width-UUYLabelwidth, frame.size.height)];
...
-(void)setYLabels:(NSArray *)yLabels
{
    ...
    CGFloat chartCavanHeight = self.frame.size.height - UULabelHeight*3;//显示的chart表格大小
    CGFloat levelHeight = chartCavanHeight /(kYLableNumbers-1+4);
   //Y轴图标
    for (int i=0; i<kYLableNumbers+kYExtralLabelNum*2; i++)
    {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake(0.0,chartCavanHeight-i*levelHeight+UULabelHeight/2, UUYLabelwidth, UULabelHeight)];
...
//        label.backgroundColor = [UIColor yellowColor];
    ...
    for (int i=0; i<kYLableNumbers+kYExtralLabelNum*2; i++)
    {
            ...
            [path addLineToPoint:CGPointMake(self.frame.size.width-5,UULabelHeight+i*levelHeight)];
            [path closePath];
...
-(void)setXLabels:(NSArray *)xLabels
{
    ...
    for (int i=0; i<xLabels.count; i++) {
        UUChartLabel * label = [[UUChartLabel alloc] initWithFrame:CGRectMake((i *  _xLabelWidth ), myScrollView.frame.size.height-UULabelHeight*3, _xLabelWidth, UULabelHeight)];
...
{
    UUChartLabel * leftlabel = [[UUChartLabel alloc] initWithFrame:CGRectMake(kScreenWidth/2.0-2*_xLabelWidth, self.frame.size.height-UULabelHeight, _xLabelWidth/2, UULabelHeight)];
    leftlabel.backgroundColor = [_colors firstObject];
    [self addSubview:leftlabel];
    UUChartLabel * leftTextlabel = [[UUChartLabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(leftlabel.frame), self.frame.size.height-UULabelHeight, _xLabelWidth, UULabelHeight)];
    leftTextlabel.text = @"收缩压";
    [self addSubview:leftTextlabel];
    //
    UUChartLabel * rightLabel = [[UUChartLabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(leftTextlabel.frame)+xLabelMargin, self.frame.size.height-UULabelHeight, _xLabelWidth/2, UULabelHeight)];
    rightLabel.backgroundColor = [_colors lastObject];
    [self addSubview:rightLabel];
    UUChartLabel * rightTextLabel = [[UUChartLabel alloc] initWithFrame:CGRectMake(CGRectGetMaxX(rightLabel.frame), self.frame.size.height-UULabelHeight, _xLabelWidth, UULabelHeight)];
...
-(void)strokeChart
{
    ...
    for (int i=0; i<_yValues.count; i++) {
        ...
        for (int j=0; j<childAry.count; j++) {
            ...
            UUBar * bar = [[UUBar alloc] initWithFrame:CGRectMake((j+(_yValues.count==1?0.1:0.05))*_xLabelWidth +i*_xLabelWidth * 0.47, 0, _xLabelWidth * (_yValues.count==1?0.8:0.45), chartCavanHeight)];
            if (_yValues.count>1) { //校准图像位置与横线位置出现的偏差
                bar.yInsetChart = UULabelHeight/2.0;
            }else{
                bar.yInsetChart = UULabelHeight+UULabelHeight/6.0;
            }
